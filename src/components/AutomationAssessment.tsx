...
const BLANK_PROBLEM = { what: "", who: "", ideal: "" };
...
export function AutomationAssessment() {
  ...
  const [s4, setS4] = useState({ problems: [{ ...BLANK_PROBLEM }] });
  ...
  const addProblem = () => {
    if (s4.problems.length < 3) {
      setS4((prev) => ({ problems: [...prev.problems, { ...BLANK_PROBLEM }] }));
    }
  };
    ...
    <div ref={topRef}>
      ...
      {step === 4 && (
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6 md:p-8">
          ...
          <h2 className="mb-1 text-xl font-semibold text-white">Your biggest problem</h2>
          <p className="mb-7 text-sm text-slate-400">
            Describe the one thing that hurts the most. You can add more if you want.
          </p>
          ...
          {s4.problems.length < 3 && (
            <button
              type="button"
              onClick={addProblem}
              className="mt-4 text-sm font-medium text-slate-400 transition hover:text-yellow-300"
            >
              + Add another problem
            </button>
          )}
